...
```
```{r}
pp_expect(loo_red2_f)  # from brms
...
new_data1 %>% 
  ...
  geom_point(data = new_data1 %>% 
             ...
             mapping= aes(x = retrieval_time_months, y = predicted*100,
                          group = population,  # group aesthetics
                          shape = population,
                          color = population,
                          linetype = population   
...
```{r}
fig_viab +
  geom_text(aes(x = 50, y = 100),
            label = "italic(Y) == frac(italic(d), 1 + exp(- italic(b) %*% (italic(x) - italic(e))))",
    parse = TRUE,
    color="#333333",
    size = 4
  ) -> fig_viab1
...
ggplot(heatmap_data80, aes(x = Group2, y = Group1)) +
  ...
  geom_text(aes(label=round(Pr_G1_gt_G2,2)), size = 2.8, color = "#333333", fontface = "bold") +
  ...
  labs(
    x = "Treatment compared against (ET50)",
    y = "Treatment being evaluated (ET50)"
  ) -> fig_et_80_pair
...
ggplot(heatmap_data50, aes(x = Group2, y = Group1)) +
  ...
  scale_size_continuous(range = c(5, 9), limits = c(0, 1)) +
  guides(
    color = guide_legend(title = "Pr(T1 > T2)"),
    size = guide_legend(title = "Pr(T1 > T2)")
  ) +
  theme_foundation() +
  theme(
    axis.text.x = element_text(),
...
heatmap_data1 %>% 
  ...
  geom_text(aes(label=round(Pr_G1_gt_G2,2)), size = 2.7, color = "#333333", fontface = "bold") +
  scale_size_continuous(range = c(5, 9), limits = c(0, 1)) +
  guides(
    color = guide_legend(title = "Pr(G1 > G2)"),
    size = guide_legend(title = "Pr(G1 > G2)")
...
col1 <- fig_et_80 / fig_et_80_pair
col2 <- fig_et_50 / fig_et_50_pair
col3 <- fig_et_1 / fig_et_1_pair

# Combine columns side by side and apply annotation
final_plot <- (col1 | col2 | col3) +
  plot_annotation(tag_levels = "A")

# Save the plot
ggsave("figures/Final_figure.png", plot = final_plot, width = 16.5, height = 10)
```
