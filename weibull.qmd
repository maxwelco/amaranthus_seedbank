...
full_model_f <- bf(
  ...
  upper + slope + t50 ~ population * depth_cm,
  nl = TRUE
)
```
...
```{r}
loo(loo_full_f, loo_simple_f, loo_red1_f, loo_red2_f, loo_red3_f, loo_red4_f)
```
...
loo_compare(loo_simple_f, loo_red1_f)
```
...
loo_compare(loo_red1_f, loo_red2_f)
```
...
new_data <- expand.grid(
  ...
  population = unique(all_data$population)
#  depth_cm = unique(all_data$depth_cm),
#  location = unique(all_data$location)
)

preds <- fitted(red_m1_f, newdata = new_data, probs = c(0.025, 0.975))
...
posterior_draws <- as_draws_df(red_m1_f)
names(posterior_draws)
```

```{r}
# --- 1. Map population names to posterior draw suffixes ---
pop_map <- c(
  "PA-CC"  = "PAMCC",
  "PA-LBK" = "PAMLBK",
  "PA-CS"  = "PAMCS",
...
```{r}
# --- 2. Function to extract parameter for a given population ---
get_param_for_group <- function(draws_df, param, population) {
  # Base intercept
  base <- draws_df[[paste0("b_", param, "_Intercept")]]
  
  # Map population name
  pop_suffix <- pop_map[population]
  
  # Construct term name
  pop_term <- paste0("b_", param, "_population", pop_suffix)
  
  # If term exists, add it to intercept
  if (pop_term %in% names(draws_df)) {
    param_value <- base + draws_df[[pop_term]]
  } else {
    param_value <- base
  }
...
# --- 5. Compute ETs by population ---
et_by_group_all <- map_dfr(group_levels, function(pop) {
  
  # Extract posterior draws for this population
  upper <- get_param_for_group(posterior_draws, "upper", pop)
  slope <- get_param_for_group(posterior_draws, "slope", pop)
  t50   <- get_param_for_group(posterior_draws, "t50", pop)
...
fig_viab +
  ...
  filter(ET_target == "ET50",
         population=="PA-CC"), aes(xintercept=median_ET))
```
...
# --- 2. Helper function to compute ET draws for a given population ---
get_ET_draws <- function(population, Y_target) {
  upper <- get_param_for_group(posterior_draws, "upper", population)
  slope <- get_param_for_group(posterior_draws, "slope", population)
  t50   <- get_param_for_group(posterior_draws, "t50", population)
  
  t50 - (1 / slope) * log((upper / Y_target) - 1)
}


# --- 3. Compute posterior ET draws for both populations ---
ET_a <- get_ET_draws(pop_a, Y_targets[ET_to_compare])
ET_b <- get_ET_draws(pop_b, Y_targets[ET_to_compare])
...
cat(sprintf("Comparison at %s:\n%s vs %s\n", 
            ET_to_compare, pop_a, pop_b))
cat(sprintf("Mean difference: %.2f\n", mean_diff))
cat(sprintf("95%% credible interval: [%.2f, %.2f]\n", ci_diff[1], ci_diff[2]))
cat(sprintf("Pr(%s > %s): %.1f%%\n", 
            pop_a, pop_b, 100 * prob_a_gt_b))
```

```{r}
# Build full posterior ET draws (not just summaries)
et_draws_long <- map_dfr(unique(all_data$population), function(pop) {
  upper <- get_param_for_group(posterior_draws, "upper", pop)
  slope <- get_param_for_group(posterior_draws, "slope", pop)
  t50   <- get_param_for_group(posterior_draws, "t50",   pop)
  ...
  tibble(
    ...
    pivot_longer(cols = starts_with("ET"), names_to = "ET_target", values_to = "ET_value")
})
...
ggplot(heatmap_data80, aes(x = Group2, y = Group1)) +
  ...
  labs(
    x = "Population compared against",
    y = "Population being evaluated"
  )
...
#    vjust = -0.8
 # ) +
  ...
  labs(
    title = "",
    x = "Treatment compared against",
    y = "Treatment being evaluated"
  )




ggsave("figures/ed50_pairwise_heatmap.png", width = 7, height = 6)
```
...
```{r}
# Add group label
ed1_wide <- ed_draws_long %>%
  filter(ET_target == "ED1") %>%
#  mutate(group = paste0(species, " - ", location)) %>%
  mutate(group = case_when(
  species == "Palmer amaranth" & location == "Lubbock" ~ "PA-LBK",
  species == "Palmer amaranth" & location == "College Station" ~ "PA-CS",
...
```{r}
group_ids1 <- colnames(ed1_matrix)

pairwise_probs1 <- matrix(NA, nrow = length(group_ids1), ncol = length(group_ids1),
                         dimnames = list(group_ids1, group_ids1))

for (i in seq_along(group_ids1)) {
  for (j in seq_along(group_ids1)) {
    if (i != j) {
      diff <- ed1_matrix[[i]] - ed1_matrix[[j]]
...
ggsave("figures/ed1_pairwise_heatmap.png", width = 7, height = 6)
```
