...
phase_2_1 %>% 
  ...
  mutate_if(is_character, as_factor) %>% 
  rename(retrieval_time_months = retrieval_time) -> phase_comb
```
...
phase2_dt %>% 
  ggplot(aes(x=retrieval_time_months, y=viable_seed_as_percent_of_original,
             color=biotype)) +
  geom_point() +
  facet_wrap(species~location~depth_cm) +
...
```{r}
phase2_dt %>% filter(location=="Lubbock") -> lub_dt
```
...
```{r}
all_data %>% filter(location=="Lubbock") -> lub_dt
```

College Station 

```{r}
all_data %>% filter(location=="College Station") -> cs_dt
```
...
```{r}
all_data %>% filter(location=="Corpus Christi") -> cc_dt
```
