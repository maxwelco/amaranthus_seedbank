...
lub_dt %>% 
  ...
  geom_point(data = pm_lub %>% 
             ...
             mapping= aes(x = retrieval_time_months, y = .fitted), color="#0072B2") +
...
ggsave("figures/Combined Lubbock.png", width = 10, height = 5)
```
...
#  coord_trans(x= "log") +
  ...
  geom_point(data = pm_cs %>% 
               mutate(retrieval_time_months = round(retrieval_time_months,0)) %>% 
               filter(retrieval_time_months %in% c(0,6,12,24,36,48,60)), 
             mapping= aes(x = retrieval_time_months, y = .fitted, color=species)) +
  theme_foundation() +
...
ED(cs_red1, c(1, 10,50,90, 99), interval = "delta", type = "absolute") %>% 
  ...
  mutate_if(is.double, ~round(.,1)) %>% 
  ggplot(aes(y=ed, x=estimate, color = species, fill = species, label = estimate)) +
  geom_point(position = position_dodge2(width = 0.7)) +
  geom_linerange(aes(xmin=lower, xmax = upper), position = position_dodge2(width = 0.7), 
                 size = 1.2) +
  geom_text(position = position_dodge2(width = 0.7), 
            vjust = 1.7, size = 3, show.legend = FALSE) +
  scale_color_colorblind() +
  scale_fill_colorblind() +
  facet_grid(~location) +
  scale_x_continuous(limits=c(0,20), breaks = seq(0,20, by=5)) + 
...
ggsave("figures/Combined College Station.png", width = 10, height = 5)
```
...
#  coord_trans(x= "log") +
  ...
  geom_ribbon(data = pm_cc, mapping=aes(x = retrieval_time_months, y = .fitted, 
                                         ymin=.lower, ymax=.upper, fill=species), 
              alpha=0.1) +
  geom_jitter(aes(color=species), alpha=0.1, show.legend = FALSE) +
...
ED(cc_red1, c(1, 10,50,90, 99), interval = "delta", type = "absolute") %>% 
  ...
  scale_fill_colorblind() +
  facet_grid(~location) +
  scale_x_continuous(limits=c(0,140), breaks = seq(0,125, by=25)) + 
  theme_foundation() +
  labs(x="Estimated month after burial", 
       y="Viable seeds (%)",
       color="", fill="", label = "") +
...
```{r}
fig_cc_2 +
  annotate("segment", x = 128, xend = 128, y = 0.8, yend = 1.2,
           color="red", linetype= "dotted") +
  annotate("text", x = 129,  y = 1, 
           color="#333333", label = "P < 0.00", hjust=0, size = 3) +
  annotate("segment", x = 34, xend = 34, y = 1.8, yend = 2.2,
           color="red", linetype= "dotted") +
  annotate("text", x = 36,  y = 2, 
           color="#333333", label = "P < 0.00", hjust=0, size = 3) +
  annotate("segment", x = 11, xend = 11, y = 2.8, yend = 3.2,
           color="red", linetype= "dotted") +
  annotate("text", x = 13,  y = 3, 
           color="#333333", label = "P = 0.00", hjust=0, size = 3) +
  annotate("segment", x = 8, xend = 8, y = 3.8, yend = 4.2,
           color="red", linetype= "dotted") +
  annotate("text", x = 10,  y = 4, 
           color="#333333", label = "P = 0.04", hjust=0, size = 3) +
  annotate("segment", x = 5, xend = 5, y = 4.8, yend = 5.2,
           color="red", linetype= "dotted") +
  annotate("text", x = 7,  y = 5, 
           color="#333333", label = "P = 0.22", hjust=0, size = 3) -> fig_cc_2_1
```


```{r}
EDcomp(cc_red1, c(99, 99), type = "absolute")
```
...
ggsave("figures/Combined Corpus Christi.png", width = 10, height = 5)
```
...
```{r}
qdrg(object = cc_red1, data = cc_dt)
```


```{r}
# If $contr exists, proceed:
if (!is.null(ed_test$contr)) {
  gh <- glht(mod, linfct = ed_test$contr)
  cld(gh)
} else {
  cat("No contrast matrix found in EDcomp output.\n")
}
```
